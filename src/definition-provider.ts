...
export class ScenarioDefinitionProvider implements vscode.DefinitionProvider {
    ...
    private getSpecDefinition(specName: string): vscode.Location | null {
        ...
        if (allSpecs.length === 0) {
            vscode.window.showWarningMessage(
                `BerryCrush: No OpenAPI specs loaded. Run "BerryCrush: Refresh OpenAPI" command.`
            );
        } else {
            vscode.window.showWarningMessage(
                `BerryCrush: Spec '${specName}' not found. Available: ${allSpecs.join(', ')}`
            );
        }
    ...
    private getOperationDefinition(operationId: string): vscode.Location | null {
        ...
        if (allOps.length === 0) {
            ...
            vscode.window.showWarningMessage(
                `BerryCrush: Operation '${operationId}' not found. Available: ${allOps.slice(0, 5).join(', ')}${allOps.length > 5 ? '...' : ''}`
            );
        }
    ...
    private getFragmentDefinition(fragmentName: string): vscode.Location | null {
        ...
        if (allFragments.length === 0) {
            vscode.window.showWarningMessage(`BerryCrush: No fragments loaded.`);
        } else {
            vscode.window.showWarningMessage(
                `BerryCrush: Fragment '${fragmentName}' not found. Available: ${allFragments.join(', ')}`
            );
        }
        return null;
    }
