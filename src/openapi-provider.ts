...
export class OpenApiProvider {
    ...
    private async findOpenApiFiles(workspaceRoot: string): Promise<string[]> {
        ...
        console.log(`BerryCrush: Scanning ${allYamlFiles.length} YAML/JSON files for OpenAPI specs...`);
        ...
        if (found.length === 0) {
            console.log('BerryCrush: No OpenAPI specs found. Configure berrycrush.openapi.path in settings.');
            vscode.window.showWarningMessage(
                'BerryCrush: No OpenAPI specs found. Configure "berrycrush.openapi.path" in settings.',
                'Open Settings'
            ).then(selection => {
                if (selection === 'Open Settings') {
                    vscode.commands.executeCommand('workbench.action.openSettings', 'berrycrush.openapi');
                }
            });
        } else {
            console.log(`BerryCrush: Found ${found.length} OpenAPI spec(s)`);
        }
